...
RSpec.describe "Category Banners" do
  ...
  describe "external link handling" do
    fab!(:user)
    fab!(:category_with_link) { Fabricate(:category, description: <<~HTML) }
          <p>
            Visit <a href="https://example.com/" rel="noopener nofollow ugc">Example</a>.
            Email <a href="mailto:hello@example.com">us</a>.
            Already <a href="https://elsewhere.example/" target="_blank" rel="noopener">opted-out</a>.
          </p>
        HTML
    let(:link_banner) { PageObjects::Components::CategoryBanner.new(category_with_link) }
    ...
    def stub_window_open
      page.execute_script(<<~JS)
        ...
        window.open = function (url, target) {
          window.__openedLinks.push({ url: url, target: target });
          return { focus: function () {}, opener: null };
        };
      JS
    end

    def opened_links
      page.evaluate_script("window.__openedLinks || []")
    end

    it "opens external links in a new tab when the user has the option enabled" do
      user.user_option.update!(external_links_in_new_tab: true)

      visit(category_with_link.url)
      expect(link_banner).to be_visible

      stub_window_open
      link_banner.dispatch_description_link_click("https://example.com/")

      expect(opened_links).to eq([{ "url" => "https://example.com/", "target" => "_blank" }])
    end

    it "does not open external links in a new tab when the user has the option disabled" do
      user.user_option.update!(external_links_in_new_tab: false)
      ...
      expect(opened_links).to be_empty
    end

    it "leaves mailto: links alone even when the option is enabled" do
      user.user_option.update!(external_links_in_new_tab: true)
      ...
      stub_window_open
      link_banner.dispatch_description_link_click("mailto:hello@example.com")

      expect(opened_links).to be_empty
    end

    it "skips modifier-clicks so the browser handles them natively" do
      user.user_option.update!(external_links_in_new_tab: true)
      ...
      stub_window_open
      link_banner.dispatch_description_link_click("https://example.com/", modifiers: [:meta])

      expect(opened_links).to be_empty
    end

    it "does not double-handle links that already opt into target=_blank" do
      user.user_option.update!(external_links_in_new_tab: true)
      ...
      stub_window_open
      link_banner.dispatch_description_link_click("https://elsewhere.example/")
